...
pandora_domain="${1:-pandora.eksa.test}"; shift || true
eksa_domain="$(echo -n "$pandora_domain" | sed -E 's,^[a-z0-9-]+\.(.+),\1,g')"
...
# and will redirect all *.$eksa_domain to the local pdns server.
...
# all *.$eksa_domain which arent in /etc/hosts are forward to our pdns server.
server=/$eksa_domain/127.0.0.2
...
# load the $eksa_domain zone into the database.
# NB we use 1m for testing purposes, in real world, this should probably be 10m+.
pdnsutil load-zone $eksa_domain <(echo "
\$TTL 1m
\$ORIGIN $eksa_domain. ; base domain-name
...
wget -qO- --header 'X-API-Key: vagrant' http://$pandora_ip_address:8081/api/v1/servers/localhost/zones/$eksa_domain | jq .
